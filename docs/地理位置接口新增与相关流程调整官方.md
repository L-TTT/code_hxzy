...
![img](https://wdoc-76491.picgzc.qpic.cn/MTY4ODg1NjcxNTM0OTQyMw_138240_hl2oheUh70GgD_4E_1653979474?w=1178&h=440)
...
![img](http://mmbiz.qpic.cn/mmbiz_png/nm27MmSkv24jfU2oq83KribPjNQt9icuHVkKHXUUonTsONrbEQ6z4d6eoLN7WhCMoondV8PCO24r23psuU5jsItw/0?wx_fmt=png)
...
2）第三方开发者：可通过 [apply_privacy_interface](https://developers.weixin.qq.com/doc/oplatform/openApi/OpenApiDoc/miniprogram-management/privacy-api-management/applyPrivacyInterface.html) 接口完成权限申请。
...
2）第三方开发者：需要在上传代码时通过参数在 ext.json 中声明其需调用的地理位置相关接口（配置方式：可通过 [commit](https://developers.weixin.qq.com/doc/oplatform/openApi/OpenApiDoc/miniprogram-management/code-management/commit.html) 接口配置）。
...
2）第三方开发者：若无需使用，可在提审时通过参数声明不使用该接口（声明方式：可通过 [submit_audit](https://developers.weixin.qq.com/doc/oplatform/openApi/OpenApiDoc/miniprogram-management/code-management/submitAudit.html) 接口配置），即可正常进行代码提审，审核通过后发布上线，将对其声明不使用的接口关闭使用权限。
